...
func main() {
	...
	err := run(ctx, config)
	if err != nil {
		log.Fatal(ctx, "error running", err)
		os.Exit(1)
	}
...
func run(ctx context.Context, config runConfig) error {
	...
	if err != nil {
		log.Error(ctx, "error reading queries from file", err)
		return err
	}

	csvFile, err := createOutputCSVFile(ctx)
	if err != nil {
		log.Error(ctx, "error creating output csv file", err)
		return err
	}
	defer func() {
		if err = csvFile.Close(); err != nil {
			log.Error(ctx, "failed closing csv file", err)
			return
		}
	}()
	...
	for _, query := range listOfQueries {
		searchResp, err := getQueryResults(ctx, query, "false", config)
		if err != nil {
			log.Error(ctx, "error getting query results", err, log.Data{"query": query})
			return err
		}
		err = AddResultsToCSV(ctx, query, csvFile, "false", searchResp)
		if err != nil {
			log.Error(ctx, "error adding results to csv", err)
			return err
		}
		searchResp, err = getQueryResults(ctx, query, "true", config)
		if err != nil {
			log.Error(ctx, "error getting query results", err, log.Data{"query": query})
			return err
		}
		err = AddResultsToCSV(ctx, query, csvFile, "true", searchResp)
		if err != nil {
			log.Error(ctx, "error adding results to csv", err)
			return err
		}
...
func createOutputCSVFile(ctx context.Context) (*os.File, error) {
	logData := log.Data{"output_filename": "top-search-query-results.csv"}
	csvFile, err := os.Create("top-search-query-results.csv")
	if err != nil {
		log.Error(ctx, "error creating csv file", err)
		return nil, err
	}
	log.Info(ctx, "successfully created csv file", logData)
	err = writeHeaderRow(csvFile, ctx)
	if err != nil {
		log.Error(ctx, "error writing header row to csv file", err)
		return nil, err
	}
	return csvFile, nil
}
...
func getQueryResults(ctx context.Context, querySupplied, nlpWeighting string, cfg runConfig) (searchResponse, error) {
	sdkItems, err := callSearchAPI(ctx, querySupplied, nlpWeighting, cfg.APIURL, cfg.NumResults)
	if err != nil {
		log.Error(ctx, "error calling SearchAPI", err)
...
func AddResultsToCSV(ctx context.Context, querySupplied string, csvFile *os.File, nlpWeighting string, responseObject searchResponse) error {
	now := time.Now()
	...
	for i := 0; i < len(responseObject.Items); i++ {
		...
		err := addItemToCSV(querySupplied, nlpOnOrOff, dateTimeRequest, item, i, csvWriter)
		if err != nil {
			log.Error(ctx, "error adding item to csv", err)
			return err
		}
	}
	return nil
...
func addItemToCSV(querySupplied, nlpOnOrOff, dateTimeRequest string, item searchItem, position int, csvWriter *csv.Writer) error {
	rowNum := position + 1
	resultsRow := []string{
		nlpOnOrOff,
		dateTimeRequest,
	...
	return csvWriter.Write(resultsRow)
}
...
func callSearchAPI(ctx context.Context, query, nlpWeighting, apiURL string, numResults int) ([]models.Item, error) {
	...
	if err != nil {
		log.Error(ctx, "failed calling Search API", err, logData)
		return nil, err
	}
...
func readQueriesFromFile(ctx context.Context, cfg runConfig) (listOfQueries []string, err error) {
	...
	if err != nil {
		log.Error(ctx, "failed opening input file", err, logData)
		return nil, err
	}
	defer func() {
		err := file.Close()
		if err != nil {
			log.Error(ctx, "failed closing input file", err, logData)
		}
	}()
	...
	if err := scanner.Err(); err != nil {
		log.Error(ctx, "failed reading input file", err, logData)
		return nil, err
	}
...
func writeHeaderRow(csvFile *os.File, ctx context.Context) error {
	headerRow := []string{
		"NLP on or off?",
	...
	if err != nil {
		log.Error(ctx, "failed writing header row", err, log.Data{"headerRow": headerRow})
		return err
	}
	return nil
}
